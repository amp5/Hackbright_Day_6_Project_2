...
def word_count():
    ...
    # unwanted_puncs = ['!', '?', ',', '.', '-', '--'] # punct we do not want
    for line in open_text:
        words_on_line = line.rstrip().replace(',', '').split(" ") # this works for getting rid of commas
        # possible list comprehension idea for getting rid of punctuation: 
        # w for w in words_on_line if in unwanted_puncs...
        # we need to be testing the last char of string w in words_on_line
        #words_on_line_vrs_2 = line.rstrip(",") # pulling all the words out of a line and putting them in a list, assigned to variable
        ...
        for word in words_on_line:
            all_words.append(word) # this adds each word from the line list to the all_words list.
                ...
                # like a list comprehension. append all words. + rstrip and split?
